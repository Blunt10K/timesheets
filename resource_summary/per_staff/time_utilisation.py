...
dates = df[['Date','year_month']].sort_values('Date')['year_month'].unique()

resources = st.multiselect("Resource",names,names)
dates = st.multiselect("Month",dates,dates[:10])

fig = time_utilisation(df[(df['Resource'].isin(resources)) & (df['year_month'].isin(dates))])
